import { TASK, EVENT, TaskDataTypeMap, CommissionPhaseT, CommissionPhaseType } from '@server/constants'
...
export default function(queue) {
    ...
    queue.process(TASK.commission_refund, async job => {
      ...
      const { commission, phase } = job.data as TaskDataTypeMap<TASK.commission_refund>
    ...
    queue.process(TASK.commission_deadline, async job => {
      ...
      let done_stages: CommissionProcessType[] = comm.phases.filter(v => v.done)
