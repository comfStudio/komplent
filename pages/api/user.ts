...
export default with_middleware(
    async (req: ExApiRequest, res: NextApiResponse) => {
        ...
        if (q.username || q.email) {
            if (
                RESERVED_USERNAMES.includes(q.username as string) ||
                (await User.check_exists(q))
            ) {
                return res
                    .status(OK)
                    .json(Object.assign(data_message('User found')))
            }
        } else if (req.user) {
            return res
                .status(OK)
                .json(
                    Object.assign(data_message('User found'), {
                        user: req.user,
                    })
                )
        }
